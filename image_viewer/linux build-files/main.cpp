...
int main(){
	...
	//text initialisation
	SDL_Surface* textsurface = TTF_RenderText_Blended(font,file,strlen(file),color);
	...
	while(running){
		...
		if(mouse_button_state && button_state){
            ...
            texture = SDL_CreateTextureFromSurface(renderer,surface);
                 textsurface = TTF_RenderText_Blended(font,file,strlen(file),color);
	             texttexture = SDL_CreateTextureFromSurface(renderer,textsurface);
	             textrec = {(float)0, (float)10, textsurface->w-10, textsurface->h-10};
            SDL_DestroySurface(surface);
            SDL_DestroySurface(textsurface);
            mouse_button_state = false;
            button_state = false;
		...
		slider(10,h-10,renderer,imagesize);
		SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255);
